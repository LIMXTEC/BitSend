...
class BitsendTestFramework(metaclass=BitsendTestMetaClass):
    ...
    def main(self):
        ...
        self.options.bitsendd = os.getenv("BITSENDD", default=config["environment"]["BUILDDIR"] + '/src/bitsendd' + config["environment"]["EXEEXT"])
        self.options.bitsendcli = os.getenv("BITSENDCLI", default=config["environment"]["BUILDDIR"] + '/src/bitsend-cli' + config["environment"]["EXEEXT"])
    ...
    def add_nodes(self, num_nodes, extra_args=None, *, rpchost=None, binary=None):
        ...
        if binary is None:
            binary = [self.options.bitsendd] * num_nodes
        assert_equal(len(extra_confs), num_nodes)
        assert_equal(len(extra_args), num_nodes)
        assert_equal(len(binary), num_nodes)
        for i in range(num_nodes):
            self.nodes.append(TestNode(i, get_datadir_path(self.options.tmpdir, i), rpchost=rpchost, timewait=self.rpc_timewait, bitsendd=binary[i], bitsend_cli=self.options.bitsendcli, mocktime=self.mocktime, coverage_dir=self.options.coveragedir, extra_conf=extra_confs[i], extra_args=extra_args[i], use_cli=self.options.usecli))
    ...
    def _initialize_chain(self):
        ...
        if create_cache:
            ...
            for i in range(MAX_NODES):
                ...
                args = [self.options.bitsendd, "-datadir=" + datadir]
                if i > 0:
                    args.append("-connect=127.0.0.1:" + str(p2p_port(0)))
                self.nodes.append(TestNode(i, get_datadir_path(self.options.cachedir, i), extra_conf=["bind=127.0.0.1"], extra_args=[], rpchost=None, timewait=self.rpc_timewait, bitsendd=self.options.bitsendd, bitsend_cli=self.options.bitsendcli, mocktime=self.mocktime, coverage_dir=None))
...
def skip_if_no_bitsendd_zmq(test_instance):
