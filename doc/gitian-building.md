...
*Setup instructions for a gitian build of Bitsend using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Bitsend
...
- [Building Bitsend](#building-bitsend)
...
Clone the git repositories for bitsend and gitian,
...
In [doc/release-process.md](release-process.md) in the bitsend repository under 'Fetch and build inputs'.
...
To build Bitsend (for Linux and/or Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the bitsend repository.

Output from `gbuild` will look something like

    Initialized empty Git repository in /home/debian/gitian-builder/inputs/bitsend/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/bitsend/bitsend
...
For example:
```bash
URL=https://github.com/laanwj/bitsend.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit bitsend=${COMMIT} --url bitsend=${URL} ../bitsend/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bitsend=${COMMIT} --url bitsend=${URL} ../bitsend/contrib/gitian-descriptors/gitian-win.yml
```
...
```bash
    gpg --detach-sign ${VERSION}/${SIGNER}/bitsend-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bitsend-build.assert
```
...
[bitsend/gitian.sigs](https://github.com/bitsend/gitian.sigs/) repository, or
