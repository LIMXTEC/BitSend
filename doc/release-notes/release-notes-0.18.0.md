BitSend Core version 0.18.0 is now available from:

  <https://bitsendcore.org/bin/bitsend-core-0.18.0/>
...
`/Applications/BitSend-Qt` (on Mac) or `bitsendd`/`bitsend-qt` (on
Linux).
...
BitSend Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use BitSend Core on unsupported systems.
...
10.10. Additionally, BitSend Core does not yet change appearance when
...
`contrib/init/bitsendd.service`, it has been changed to use
`/var/lib/bitsendd` as the data directory instead of
`~bitsend/.bitsend`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/bitsendd` will
exist has enough space (check using `df -h /var/lib/bitsendd`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/bitsend/bitsend/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://bitsendcore.org/en/contact/) the BitSend Core
...
The systemd init file (`contrib/init/bitsendd.service`) has been changed
to use `/var/lib/bitsendd` as the data directory instead of
`~bitsend/.bitsend`. This change makes BitSend Core more consistent with
...
`datadir` option in `/etc/bitsend/bitsend.conf` will have no effect.
...
- A new short [document](https://github.com/bitsend/bitsend/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/bitsend/bitsend/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/bitsend/bitsend/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/bitsend/bitsend/blob/master/doc/bitsend-conf.md)
  about the `bitsend.conf` file describes how to use it to configure
  BitSend Core.

- A new document introduces BitSend Core's BIP174 [Partially-Signed
  BitSend Transactions
  (PSBT)](https://github.com/bitsend/bitsend/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/bitsend/bitsend/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent BitSend-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  BitSend Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/bitsend/bitsend/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/bitsend/bitsend/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when BitSend Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of BitSend Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/bitsend/bitsend/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/bitsend/bitsend/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart bitsendd with the `-deprecatedrpc=generate`
...
- A new `bitsend-wallet` tool is now distributed alongside BitSend
...
This section describes planned changes to BitSend Core that may affect
other BitSend software and services.

- Since version 0.16.0, BitSend Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with BitSend Core 0.20 (expected about
  a year after 0.18), BitSend Core will default to native segwit
  ...
  to bech32 addresses, and if the BitSend Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in BitSend Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any BitSend Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, BitSend Core
...
- A sub-project of BitSend Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with BitSend Core.  See their [project
  page](https://github.com/bitsend-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to BitSend Core's own implementation, although entropy
  gathered by BitSend Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves BitSend Core a
...
- On macOS, BitSend Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `bitsend_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libbitsend_server.a` (jnewbery)
...
- #13926 [Tools] bitsend-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like bitsendd (MarcoFalke)
...
- #14521 qt, docs: Fix `bitsend-qt -version` output formatting (hebasto)
...
- #11625 Add BitSendApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libbitsend_server` (laanwj)
...
- #15154 Configure: bitsend-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/bitsend-wallet (Sjors)
...
- #11526 Visual Studio build configuration for BitSend Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libbitsend_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in bitsend-util-test.py (isghe)
...
- #12775 Integration of property based testing into BitSend Core (Christewart)
...
- #12246 Bugfix: Only run bitsend-tx tests when bitsend-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_bitsend` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_bitsend` (MarcoFalke)
...
- #14186 bitsend-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_bitsend.conf` message on startup if conf file exists (leishman)
...
- #12255 Update bitsend.service to conform to init.md (dongcarl)
...
- #14497 Add doc/bitsend-conf.md (hebasto)
...
- #15354 Add missing `bitsend-wallet` tool manpages (MarcoFalke)
...
- #15765 correct bitsendconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support bitsend folder on OSX (JimmyMow)
