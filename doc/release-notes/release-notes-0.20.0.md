...
BitSend Core version 0.20.0 is now available from:

  <https://bitsendcore.org/bin/bitsend-core-0.20.0/>
...
using the Linux kernel, macOS 10.12+, and Windows 7 and newer.  BitSend
Core should also work on most other Unix-like systems but is not as
frequently tested on them.  It is not recommended to use BitSend Core on
unsupported systems.

From BitSend Core 0.20.0 onwards, macOS versions earlier than 10.12 are no
longer supported. Additionally, BitSend Core does not yet change appearance
...
- Instead of running `make` simply, you should instead run
  `BITSEND_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from BitSend Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since BitSend Core version 0.18.0.
...
- Testing or debugging of implementations of the BitSend P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of BitSend Core. BitSend Core logs debug messages
...
- OpenSSL is no longer used by BitSend Core.  (#17265)

- BIP70 support has been fully removed from BitSend Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/bitsend/bitsend/issues/16599),
  [PR #16702](https://github.com/bitsend/bitsend/pull/16702), and the
  `bitsendd help` for more information.  This option is experimental and
...
- All custom settings configured when BitSend Core starts are now
...
- The "Start BitSend Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://bitsendcore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple BitSend Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed BitSend Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/bitsend-core/HWI) for
  signing.  Future versions of BitSend Core should support a GUI option
...
- BitSend Core's automatically-generated source code documentation is
  now available at https://doxygen.bitsendcore.org.  (#17596)
...
- The `bitsend-cli` utility used with the `-getinfo` parameter now
...
- The `bitsend-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `bitsendstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_bitsend-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBBITSEND_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_bitsend` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBBITSEND_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for bitsend-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate bitsend schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest bitsend.conf (sdaftuar)
...
- #18022 Fix appveyor `test_bitsend` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (bitsend-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone bitsend-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about BitSend Core files (hebasto)
...
- #17119 Fix broken bitsend-cli examples (andrewtoth)
...
- #17503 Remove bitness from bitsend-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.bitsendcore.org (laanwj)
...
- #17648 Rename wallet-tool references to bitsend-wallet (hel-o)
...
- #16115 On bitsendd startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from bitsend-cli -getinfo (malevolent)
...
[Transifex](https://www.transifex.com/bitsend/bitsend/).
