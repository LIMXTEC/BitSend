...
Copy the masternode private key and correspondig collateral output transaction that holds the 1K LIMX.
...
## What about the limecoinx.conf file?

If you are using a masternode.conf file you no longer need the limecoinx.conf file. The exception is if you need custom settings (thanks oblox). 

## Update limecoinx.conf on server

If you generated a new masternode private key, you will need to update the remote limecoinx.conf files.

Shut down the daemon and then edit the file.

```sudo nano .limecoinx/limecoinx.conf```

### Edit the masternodeprivkey
If you generated a new masternode private key, you will need to update the masternodeprivkey value in your remote limecoinx.conf file.
