...
const char * const BITSEND_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/bitsend/bitsend/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bitsend-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", BITSEND_SETTINGS_FILENAME);
...
{
    // Windows: C:\Users\Username\AppData\Roaming\BitSend
    // macOS: ~/Library/Application Support/BitSend
    // Unix-like: ~/.bitsend
#ifdef WIN32
    // Windows
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", BITSEND_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure BitSend Core copyright is not removed by accident
    if (copyright_devs.find("BitSend Core") == std::string::npos) {
