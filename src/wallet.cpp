        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                BOOST_FOREACH (const PAIRTYPE(CScript, CAmount)& s, vecSend)
                {
                    ...
                    if (txout.IsDust(::minRelayTxFee))//if (txout.IsDust(CTransaction::nMinRelayTxFee))
                    {
               ...
               /* if (nFeeRet < CTransaction::nMinTxFee && nChange > 0 && nChange < CENT)
                ...
                if (nChange > 0)
                {
                    ...
                    if (newTxOut.IsDust(::minRelayTxFee))//if (newTxOut.IsDust(CTransaction::nMinRelayTxFee))
                    {
                        nFeeRet += nChange;
