...
void ParseParameters(int argc, const char* const argv[])
{
    ...
    BOOST_FOREACH(const PAIRTYPE(string,string)& entry, mapArgs)
    {
        ...
        if (name.find("--") == 0)
        {
            std::string singleBitsend(name.begin()+1, name.end());
            if (mapArgs.count(singleBitsend) == 0)
                mapArgs[singleBitsend] = entry.second;
            name = singleBitsend;
        }
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "bitsend.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "bitsendd.pid"));
...
void AddTimeData(const CNetAddr& ip, int64_t nTime)
{
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        // Old (abs64(nMedian) < 70 * 60) Bitsenddev 31-05-2015
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Bitsend will not work properly.");
