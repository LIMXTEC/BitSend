...
void ParseParameters(int argc, const char* const argv[])
{
    ...
    BOOST_FOREACH(const PAIRTYPE(string,string)& entry, mapArgs)
    {
        ...
        if (name.find("--") == 0)
        {
            std::string singleLimecoinx(name.begin()+1, name.end());
            if (mapArgs.count(singleLimecoinx) == 0)
                mapArgs[singleLimecoinx] = entry.second;
            name = singleLimecoinx;
        }
...
#else
    const char* pszModule = "limecoinx";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Limecoinx
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Limecoinx
    // Mac: ~/Library/Application Support/Limecoinx
    // Unix: ~/.limecoinx
#ifdef WIN32
    // Windows
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "limecoinx.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty limecoinx.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override limecoinx.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "limecoinxd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Limecoinx will not work properly.");
