...
{	FILE* filestr = fsbridge::fopen(GetDataDir() / "mempool.dat", "rb");
    ...
    {
        //LOCK(pool.cs);//todo++
        CCoinsViewMemPool viewMemPool(pcoinsTip.get(), mempool);//CCoinsViewMemPool viewMempool(pcoinsTip, mempool);
 ...
 bool AcceptableInputs(CTxMemPool& pool, CValidationState &state, const CTransactionRef& ptx, bool ignoreFees)
{
	...
	
	{
		...
		BOOST_FOREACH(const CTxIn txin, tx.vin) {
        bool fHadTxInCache = pcoinsTip->HaveCoinInCache(txin.prevout);
        if (view.HaveCoin(txin.prevout)) {
            if (!fHadTxInCache)
        ...
        // This is done last to help prevent CPU exhaustion denial-of-service attacks.
        PrecomputedTransactionData txdata(tx);
        if (!CheckInputs(tx, state, view, false, SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_STRICTENC, true,false, txdata)) {
...
bool static FlushStateToDisk(const CChainParams& chainparams, CValidationState &state, FlushStateMode mode, int nManualPruneHeight) {
    ...
    bool fCacheLarge = mode == FLUSH_STATE_PERIODIC && cacheSize > std::max((9 * nTotalSpace) / 10, nTotalSpace - 100 * 1024 * 1024);// The cache is over the limit, we have to write now.
...
bool DisconnectBlocksAndReprocess(int blocks)
{
	...
	{CChainState DisconnectTip;DisconnectTip.DisconnectTip(state, chainparams, &disconnectpool);//todo++}
	}
    return true;
}
            ...
            if(pindex->GetBlockHash() == block.hashPrevBlock){
                ...
                if (!fIsInitialDownload && pindex != NULL)
                {
                    ...
                    if(!foundPaymentAndPayee) {
                        //LogPrintf("CheckBlock() : !!Couldn't find masternode payment(%d|%d) or payee(%d|%s) nHeight %d. \n", foundPaymentAmount, masternodePaymentAmount, foundPayee, EncodeDestination(address1)/* address2.ToString().c_str() */, chainActive.Tip()->nHeight+1);
        ...
        while (num--) {
            ...
            if (nTime + nExpiryTimeout > nNow) {
                ...
                AcceptToMemoryPoolWithTime(chainparams, mempool, state, tx, nullptr /* pfMissingInputs */, nTime,
                                           nullptr /* plTxnReplaced */, false /* bypass_limits */, 0 /* nAbsurdFee */,1);
                if (state.IsValid()) {
                    ++count;
                } else {
