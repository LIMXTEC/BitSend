...
CAmount AmountFromValue(const Value& value)
{
    ...
    if (dAmount <= 0.0 || dAmount > 525000000.0)  //bitsenddev 04-2015 old 21000000
...
Value stop(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "Bitsend server stopping";
}
...
void StartRPCThreads()
{
         ...
         (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"])) && Params().RequireRPCPassword())
    {
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("To use bsddeamon, or the -server option to bitsend-qt, you must set an rpcpassword in the configuration file:\n"
              "%s\n"
...
std::string HelpExampleCli(string methodname, string args){
    return "> bitsend-cli " + methodname + " " + args + "\n";
}
