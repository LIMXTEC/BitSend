...
int64_t AmountFromValue(const Value& value)
{
    ...
    if (dAmount <= 0.0 || dAmount > 525000000.0)  //limxdev 04-2015 old 21000000
...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "\nStop Limecoinx server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "Limecoinx server stopping";
}
...
void StartRPCThreads()
{
         ...
         (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"])) && Params().RequireRPCPassword())
    {
        ...
        string strWhatAmI = "To use limecoinxd";
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("%s, you must set a rpcpassword in the configuration file:\n"
              ...
              "for example: alertnotify=echo %%s | mail -s \"Limecoinx Alert\" admin@foo.com\n"),
                strWhatAmI,
...
std::string HelpExampleCli(string methodname, string args){
    return "> limecoinx-cli " + methodname + " " + args + "\n";
}
