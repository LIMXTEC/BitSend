...
bool parseBitSendURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BitSendUnits::parse(BitSendUnits::BTC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
QString formatBitSendURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("bitsend:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BitSendUnits::format(BitSendUnits::BTC, info.amount, false, BitSendUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", BITSEND_CONF_FILENAME));
...
fs::path static GetAutostartFilePath()
{
    ...
    return GetAutostartDir() / strprintf("bitsend-%s.desktop", chain);
}
