...
ClientModel is used by the main application `bitsendgui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/bitsend/bitsend/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a bitsend:... link from a browser).
...
* don't change `local/bitsend_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "bitsend-qt" under "Run", which is where you can also add command line arguments)
