...
BitSendCore::BitSendCore(interfaces::Node& node) :
    QObject(), m_node(node)
{
}
...
static const char* qt_argv = "bitsend-qt";
...
void BitSendApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the BitSendApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", BitSendGUI::DEFAULT_UIPLATFORM);
...
{
    window = new BitSendGUI(node(), platformStyle, networkStyle, nullptr);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &BitSendGUI::detectShutdown);
}
...
void BitSendApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    ...
    m_splash->show();
    connect(this, &BitSendApplication::requestedInitialize, m_splash, &SplashScreen::handleLoadWallet);
    connect(this, &BitSendApplication::splashFinished, m_splash, &SplashScreen::finish);
    connect(this, &BitSendApplication::requestedShutdown, m_splash, &QWidget::close);
}

void BitSendApplication::setNode(interfaces::Node& node)
{
    assert(!m_node);
    m_node = &node;
...
bool BitSendApplication::baseInitialize()
{
...
void BitSendApplication::startThread()
{
    ...
    BitSendCore *executor = new BitSendCore(node());
    executor->moveToThread(coreThread);

    /*  communication to and from thread */
    connect(executor, &BitSendCore::initializeResult, this, &BitSendApplication::initializeResult);
    connect(executor, &BitSendCore::shutdownResult, this, &BitSendApplication::shutdownResult);
    connect(executor, &BitSendCore::runawayException, this, &BitSendApplication::handleRunawayException);
    connect(this, &BitSendApplication::requestedInitialize, executor, &BitSendCore::initialize);
    connect(this, &BitSendApplication::requestedShutdown, executor, &BitSendCore::shutdown);
...
void BitSendApplication::InitializePruneSetting(bool prune)
{
...
void BitSendApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success)
    {
        ...
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &BitSendGUI::handlePaymentRequest);
            connect(window, &BitSendGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void BitSendApplication::shutdownResult()
{
...
{
    QMessageBox::critical(nullptr, "Runaway exception", BitSendGUI::tr("A fatal error occurred. %1 can no longer continue safely and will quit.").arg(PACKAGE_NAME) + QString("<br><br>") + message);
    ::exit(EXIT_FAILURE);
}
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", BitSendGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
