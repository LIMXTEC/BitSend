...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"     (string, required) The bitsend address to validate\n"
            "\nResult:\n"
            "{\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Bitsend address and we have full public key:
        CBitsendAddress address(ks);
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are bitsend addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) bitsend address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
    ...
    CBitsendAddress address(innerID);
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The bitsend address to use for the signature.\n"
...
UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw runtime_error(
            ...
            "bitsend-cli and the GUI. There is no server-side difference."
        );
...
{
    if (type == 2) {
        address = CBitsendAddress(CScriptID(hash)).ToString();
    } else if (type == 1) {
        address = CBitsendAddress(CKeyID(hash)).ToString();
    } else {
        return false;
    }
    return true;
...
bool getAddressesFromParams(const UniValue& params, std::vector<std::pair<uint160, int> > &addresses)
{
    if (params[0].isStr()) {
        ...
        for (std::vector<UniValue>::iterator it = values.begin(); it != values.end(); ++it) {
            ...
            CBitsendAddress address(it->get_str());
            uint160 hashBytes;
            int type = 0;
            if (!address.GetIndexKey(hashBytes, type)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
