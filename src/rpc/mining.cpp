...
UniValue masternodelist(const JSONRPCRequest& request)
{
    ...
    if (strMode == "rank") {
        ...
        BOOST_FOREACH(PairType& s, vMasternodeRanks) {
        ...
        BOOST_FOREACH(CMasternode& mn, vMasternodes) {
            ...
            if (strMode == "activeseconds") {
                ...
                if(strFilter !="" && address2.find(strFilter) == string::npos &&
                    strAddr.find(strFilter) == string::npos) continue;

                std::string strOut = "";

                if(mn.donationPercentage != 0){
                    strOut = address2.c_str();
                    strOut += ":";
                ...
                if(strFilter !="" && address2.find(strFilter) == string::npos &&
                    strAddr.find(strFilter) == string::npos) continue;
                obj.push_back(Pair(strAddr,       address2.c_str()));
...
UniValue masternode(const JSONRPCRequest& request)
{
    ...
    if (strCommand == "current")
    {
        ...
        if(winner) {
            ...
            obj.push_back(Pair("pubkey",        address2.c_str()));
	...
	if (strCommand == "winners")
    {
        ...
        for(int nHeight = chainActive.Tip()->nHeight-10; nHeight < chainActive.Tip()->nHeight+20; nHeight++)
        {
            ...
            if(masternodePayments.GetBlockPayee(nHeight, payee)){
                ...
                //CBitsendAddress address2(address1);
				std::string address2 = EncodeDestination(address1);
                obj.push_back(Pair(boost::lexical_cast<std::string>(nHeight),       address2.c_str()));
            } else {
