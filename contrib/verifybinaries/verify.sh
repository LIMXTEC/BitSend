...
if [ $? -ne 0 ]; then
   echo "bitsend.org failed to provide signature file, but bitsendcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bitsend.org and bitsendcore.org signature files were not equal?"
